...
const FuncionarioSearchTable = forwardRef((props, ref) => { // forwardRef is used to update method from this file from ather files
    ...
    useEffect(() => {
        getGetAllDataFuncionarioAgencia();
    }, []);
    ...
    const getGetAllDataFuncionarioAgencia = () => {

        FuncionarioService.getAll(0, 0, "TodasAsAgencia", "").then(response => {
    ...
    const columns = [
        ...
        {
            field: 'total_agencias', headerName: t('total_employes'), flex: 0.4, headerClassName: classes.paper,
            renderCell: (params) => {
                return (
                    <>
                        <h4 style={{fontWeight:"bold", textAlign :"center", margin: "0 auto"}}>
                            {params.row.total_agencias}</h4>
                    </>
                )
            }
        },
