...
const RatingSearchTable = forwardRef((props, ref) => { // forwardRef is used to update method from this file from ather files
    ...
    const getGetAllData = (sedeID, rating) => {
        ...
        try{
            RatingServices.getAll(sedeID,rating )
            .then(response => {
    ...
    const columns = [
        ...
        descricaoDisplay ?
            { field: 'descricao', headerName: t('descricao_avaliacao'), flex: 3, headerClassName: classes.paper } :
            { field: 'descricao', headerName: 'descricao_avaliacao', hide: { descricaoDisplay }, headerClassName: classes.paper },
