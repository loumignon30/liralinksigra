...
const FuncaoResumoSearchTable = forwardRef((props, ref) => { // forwardRef is used to update method from this file from ather files
    ...
    useEffect(() => {
        getGetAllDataFuncionarioFuncao(sedeID, 0);
    }, []);

    useImperativeHandle(ref, () => (
        {
            getGetAllDataFuncionarioFuncao: getGetAllDataFuncionarioFuncao
        }
    ));

    const getGetAllDataFuncionarioFuncao = (sedeID1) => {

        FuncionarioService.getAll(sedeID1, 0, "resumoFuncao").then(response => {
    ...
    const columns = [
        ...
        {
            field: 'funcao', headerName: t('funcao'), flex: 1, headerClassName: classes.paper,
            renderCell: (params) => {
                return (
                    <>
                        {params.row.funcaoFuncionario.funcao}
                    </>
                )
            }
        },
        ...
        {
            field: 'total_por_Funcao', headerName: t('total_employes'), flex: 0.4, headerClassName: classes.paper,
            renderCell: (params) => {
                return (
                    <>
                        <h4 style={{fontWeight:"bold", textAlign :"center", margin: "0 auto"}}>
                            {params.row.total_por_Funcao}</h4>
                    </>
                )
            }
        },
...
export default FuncaoResumoSearchTable;