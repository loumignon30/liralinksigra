...
const TipoDeDeNuncia = (props) => {
    ...
    const gavarRating = () => {
        ...
        if (validate()) {
            ...
            if (values.id > 0) {
                RatingServices.update(values.id, values).then(response => {
                    // tableTipoDenunciaUpdateData(currentLanguageCode)
                    setNotify({
                        isOpen: true,
                        message: t('mensagem_modificar_Nova_Agencia'),
                ...
                RatingServices.create(values).then(response => {
                    //  tableTipoDenunciaUpdateData(currentLanguageCode)
                    setNotify({
                        isOpen: true,
                        message: t('mensagem_Gravar_Nova_Agencia'),
    ...
    const gavarMotivoRating = () => {
        ...
        if (values.id > 0) {
            RatingMotivo.update(values.id, values).then(response => {
                // tableTipoDenunciaUpdateData(currentLanguageCode)
                setNotify({
                    isOpen: true,
                    message: t('mensagem_modificar_Nova_Agencia'),
            ...
            RatingMotivo.create(values).then(response => {
                //  tableTipoDenunciaUpdateData(currentLanguageCode)
                setNotify({
                    isOpen: true,
                    message: t('mensagem_Gravar_Nova_Agencia'),
