...
const DenunciaSearchTable = forwardRef((props, ref) => { // forwardRef is used to update method from this file from ather files
    ...
    const { idDisplay, nomeDisplay, tipoDenunciaDisplay, dataDisplay,
        ...
        pageSize, rowPerPage,
        abreviationLangue, sedeID, agenciaID, tipoMovimento, 
        ratingDisplay, ratingDescricaoDisplay, ratingMotivoDisplay, 
        trabalhadorDisplay, tipoImpressao } = props;
    ...
    useEffect(() => {
       ...
       getGetAllData( abreviationLangue,  tipoImpressao, sedeID, agenciaID, tipoMovimento, 0);
    ...
    const getGetAllData = (abreviationLangue1,  tipoImpressao1, sedeID1, agenciaID1, tipoMovimento1, funcionarioID1) => {

        // alert(abreviationLangue1);
        // alert(tipoImpressao1);
        // alert(sedeID1);
        // alert(agenciaID1);
        // alert(tipoMovimento1);
        
        DenunciaService.getAll(abreviationLangue1, tipoImpressao1, sedeID1, agenciaID1, tipoMovimento1, funcionarioID1)
            .then(response => {
    ...
    const getGetAllDataFuncionario = (sedeID1, agenciaID1, tipoMovimento1, funcionarioID1) => {
        DenunciaService.getAll("pt", "funcionario", sedeID1, agenciaID1, tipoMovimento1, funcionarioID1)
            .then(response => {
                // console.log(response.data)
                setData(response.data)
            })
    ...
    function getStatusData(idStatus) {
        const resultStatus = StatusDenunciaData().find(({ id }) => Number(id) === Number(idStatus));
    ...
    const columns = [
        ...
        }:{ field: 'id', headerName: 'ID', hide: { ratingDisplay }, headerClassName: classes.paper },
        ...
        }:  { field: 'id', headerName: 'ID', hide: { ratingDescricaoDisplay }, headerClassName: classes.paper },
        ,
        ratingMotivoDisplay ?
        {
 ...
 */}
        ...
        }:{ field: 'id', headerName: 'ID', hide: { ratingMotivoDisplay }, headerClassName: classes.paper },
        trabalhadorDisplay?
        {
        ...
        }:{ field: 'id', headerName: 'ID', hide: { trabalhadorDisplay }, headerClassName: classes.paper },
        statusDisplay ?
            {
                field: 'status', headerName: t('status'), flex: 0.6, headerClassName: classes.paper,
                renderCell: (type) => {
                    return (
                        <>
                            <button className={"ButtonStatutDataGrid " + "_" + type.row.status + "Denuncia"}>
        ...
        actionsButtonDisplaySelect ?
            {
                field: 'action', headerName: t('Tratar'), width: 80, headerClassName: classes.paper,
                renderCell: (params) => {
                    return (
                        <>
                        <Link to={"/tratamentoDenuncia/" + params.row.id}
                            state={{
                                 id: params.row.id,
                                 codigoTrabalhador: params.row.funcaionarioDenuncia.code,
                                primeironomeTrabalhador: params.row.funcaionarioDenuncia.primeironome,
                                ultimonomeTrabalhador: params.row.funcaionarioDenuncia.ultimonome,
                                telepfoneTrabalhador: params.row.funcaionarioDenuncia.telefone,
                                ...
                                status: params.row.status,
                                 nomeDenunciante: params.row.nome,
                                 imageChangeFromOutSideURL: params.row.funcaionarioDenuncia.imageName ? "https://s3.amazonaws.com/liralink.sigra/" + params.row.funcaionarioDenuncia.imageName : "https://s3.amazonaws.com/liralink.sigra/" + "semfoto.png",
